...
class Motor:
    ...
    def Temperature(self,tick):
        ...
        if not self.__torque ==0:
            ...
            self.__temp = (self.__tempAmb+ self.__temp_level) + (self.__oldTemp-(self.__tempAmb+ self.__temp_level))*\
                (math.exp(-self.__elapsedTime/self.__tal))
        else:
            self.__temp = self.__tempAmb
    ...
    def partida(self, estado, frequencia_desejada,tick):
        ...
        if self.getState():
            ...
            if self.__f < frequencia_desejada and estado is True:
                self.__f += frequencia_desejada/(self.__tstart/tick)
